...
/* Higher delay than datasheet due to HW */
#define PDN_LOW_DELAY_USEC  17000
#define PDN_HIGH_DELAY_USEC  12000

/* Workaround from TI, sleep after bclk off and before deep sleep */
#define TAS5805M_BCLK_OFF_SLEEP_USEC_MIN 100
#define TAS5805M_BCLK_OFF_SLEEP_USEC_MAX 110
...
void tas5805m_process_fault(struct snd_soc_codec *codec)
{
	...
	snd_soc_write(codec, TAS5805M_REG_00, TAS5805M_PAGE_00);
	chan_fault_reg = snd_soc_read(codec, TAS5805M_REG_70) & 0xf;
	pvdd_fault_reg = snd_soc_read(codec, TAS5805M_REG_71) & 0x3;
	oc_fault_reg = snd_soc_read(codec, TAS5805M_REG_72) & 0x1;
	if (chan_fault_reg || pvdd_fault_reg || oc_fault_reg) {
		pr_warn("%s: CHAN_FAULT (0x70) %x\n", __func__, chan_fault_reg);
		pr_warn("%s: GLOBAL_FAULT1 (0x71) %x\n", __func__, pvdd_fault_reg);
		pr_warn("%s: GLOBAL_FAULT2 (0x72) %x\n", __func__, oc_fault_reg);
		usleep_range(10000, 10200);
		snd_soc_write(codec, TAS5805M_REG_78, TAS5805M_CLEAR_FAULT);
		pr_warn("%s: Processed TAS5805M AMP fault\n", __func__);
	}
}
EXPORT_SYMBOL(tas5805m_process_fault);
...
static int tas5805m_device_state(struct snd_soc_codec *codec, int sleep)
{
	...
	if (sleep) {
		/*
		 * machine driver should turn off bclk, and need to sleep
		 * for 100us after
		 */
		usleep_range(TAS5805M_BCLK_OFF_SLEEP_USEC_MIN,
						TAS5805M_BCLK_OFF_SLEEP_USEC_MAX);
		...
		/* clear the forced clk fault by bclk off */
		status |= tas5805m_set_register(codec, TAS5805M_BOOK_00,
		TAS5805M_PAGE_00, TAS5805M_REG_78, TAS5805M_CLEAR_FAULT);
	} else {
		status = tas5805m_set_register(codec, TAS5805M_BOOK_00,
		TAS5805M_PAGE_00, TAS5805M_REG_03, TAS5805M_HIZ_STATE);
...
int tas5805m_reset_platform(struct snd_soc_codec *codec, int enable)
{
	...
	pr_info("%s-: enable %d playback_active %d %d!\n", __func__,
		enable, playback_active[0], playback_active[1]);
