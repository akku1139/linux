...
static wait_queue_head_t recovery_waitq;
static INT32 wlan_mode = WLAN_MODE_HALT;
static INT32 powered = POWER_STATE_OFF;
...
do { \
	if (g_IsNeedDoChipReset) { \
		powered = POWER_STATE_RECOVERING; \
...
INT32 wifi_reset_start(VOID)
{
	...
	if (powered == POWER_STATE_ON) {
		...
		/* WIFI is off before whole chip reset
		 * or
		 * WIFI turn on failed last time and waiting for recovery
		 * do nothing
		 */
	}
...
INT32 wifi_reset_end(ENUM_RESET_STATUS_T status)
{
	...
	if (status == RESET_FAIL) {
		if (powered == POWER_STATE_ON) {
			...
			powered = POWER_STATE_OFF;
		} else if (powered == POWER_STATE_RECOVERING) {
			/* Whole chip reset fail, donot recover WIFI, just wakeup recovery_waitq */
			powered = POWER_STATE_OFF;
			wake_up_interruptible(&recovery_waitq);
		}
		ret = 0;
		...
		if (powered == POWER_STATE_ON) {
			...
			/* WIFI turn on failed last time, try to reopen it */
			if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_WIFI)) {
				powered = POWER_STATE_OFF;
				WIFI_ERR_FUNC("WMT turn on WIFI fail!\n");
			} else {
...
ssize_t WIFI_write(struct file *filp, const char __user *buf, size_t count, loff_t *f_pos)
{
	...
	if (0 == copy_from_user(local, buf, (count > sizeof(local)) ? sizeof(local) : count)) {
		...
		if (local[0] == '0') {
			...
			if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_WIFI)) {
				WIFI_ERR_FUNC("WMT turn on WIFI failed, try to recover\n");
				WMT_CHECK_DO_CHIP_RESET();
			} else {
				powered = POWER_STATE_ON;
				retval = count;
	...
	if (powered == POWER_STATE_RECOVERING) {
		timeout = wait_event_interruptible_timeout(recovery_waitq,
			(powered != POWER_STATE_RECOVERING),
			(5 * HZ));
		if (timeout == 0) {
			powered = POWER_STATE_OFF;
			WIFI_ERR_FUNC("WIFI recovery timeout\n");
		} else if (timeout < 0) {
			powered = POWER_STATE_OFF;
			WIFI_INFO_FUNC("WIFI recovery wait error\n");
		} else {
			WIFI_INFO_FUNC("WIFI recovery finished\n");
			if (powered == POWER_STATE_ON)
				retval = count;
		}
	}
...
static int WIFI_init(void)
{
	...
	init_waitqueue_head(&recovery_waitq);
