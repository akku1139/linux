...
{
	UINT_8 *pucProcBuf = kalMemAlloc(PROC_MAX_BUF_SIZE, VIR_MEM_TYPE);
	UINT_8 *temp = NULL;
	UINT_32 u4CopySize = 0;
	UINT_16 i;
...
static ssize_t procDbgLevelWrite(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	...
	kalMemZero(pucProcBuf, u4CopySize);
	temp = pucProcBuf;
...

{
	...
	if (u2CountryCode == 0x0)
		kalStrCpy(pucProcBuf, "Country: NULL\n");
	else
				...
				size_t count, loff_t *data)
{
	...
	aucCountry[0] = pucProcBuf[0];
	aucCountry[1] = pucProcBuf[1];
	if ('X' == pucProcBuf[0] && 'X' == pucProcBuf[1])
		pucProcBuf[0] = pucProcBuf[1] = 'W';

	pucProcBuf[u4CopySize] = '\0';
	rStatus = kalIoctl(g_prGlueInfo_proc, wlanoidSetCountryCode,
				&pucProcBuf[0], 2, FALSE, FALSE, TRUE, FALSE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS) {
...
static ssize_t procRoamRead(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	rStatus = kalIoctl(g_prGlueInfo_proc, wlanoidGetRoamParams, pucProcBuf, PROC_MAX_BUF_SIZE,
...
static ssize_t procRoamWrite(struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (kalStrnCmp(pucProcBuf, "force_roam", 10) == 0)
		...
		rStatus = kalIoctl(g_prGlueInfo_proc, wlanoidSetRoamParams, pucProcBuf,
				   kalStrLen(pucProcBuf), FALSE, FALSE, TRUE, FALSE, &u4BufLen);
				   ...
				   size_t count, loff_t *data)
{
	...
	if (kalStrnCmp(pucProcBuf, opt[0], strlen(opt[0])) == 0) {
		/* ant_query */
		status = sscanf(pucProcBuf +  strlen(opt[0]), "%u %u",
		...
		rStatus = kalIoctl(g_prGlueInfo_proc, wlanoid_set_antenna_rssi,
				   ...
				   FALSE, FALSE, FALSE, FALSE, &u4BufLen);
	} else if (kalStrnCmp(pucProcBuf, opt[1], strlen(opt[1])) == 0) {
		/* ant_avg */
		status = sscanf(pucProcBuf +  strlen(opt[1]), "%hhu",
				&moving_avg_mode);
		...
		rStatus = kalIoctl(g_prGlueInfo_proc,
				   ...
				   FALSE, FALSE, FALSE, FALSE, &u4BufLen);
	} else if (kalStrnCmp(pucProcBuf, opt[2], strlen(opt[2])) == 0) {
		/* ant_dwell */
		status = sscanf(pucProcBuf +  strlen(opt[2]), "%hu",
				&dwell);
				   ...
				   size_t count, loff_t *f_pos)
{
	...
	pos += snprintf(pucProcBuf, PROC_MAX_BUF_SIZE,
			"DTIM Skip Count:%hhu\n",
			dtim_skip_count);
				     ...
				     size_t count, loff_t *data)
{
	...
	if (sscanf(pucProcBuf, "%hhu", &dtim_skip_count) == 1) {
