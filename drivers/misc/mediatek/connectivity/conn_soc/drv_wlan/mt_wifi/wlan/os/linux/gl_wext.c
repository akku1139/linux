...
static const struct iw_priv_args rIwPrivTable[] = {
	...
	{ PRIV_CMD_DTIM_SKIP_COUNT,
		IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, 0, "set_dtim_skip" },
	{ PRIV_CMD_DTIM_SKIP_COUNT, 0,
		IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_dtim_skip" },
	      ...
	      IN struct iw_request_info *prIwrInfo, IN OUT struct iw_point *prData, IN char *pcExtra)
{
	...
	if (u4AllocBufLen < sizeof(PARAM_BSSID_LIST_EX_T)) {
		DBGLOG(REQ, WARN, "[wifi] invalid input length:%d\n", prData->length);
		return -EINVAL;
	}
	...
	for (i = 0; i < prList->u4NumberOfItems; ++i) {
		kalMemZero(&iwEvent, sizeof(struct iw_event));
		/* BSSID */
...
wext_set_encode_ext(IN struct net_device *prNetDev,
		    IN struct iw_request_info *prIwrInfo, IN struct iw_point *prEnc,
		    IN char *pcExtra, IN UINT_32 u4ExtraSize)
{
	...
	if (u4ExtraSize < (sizeof(struct iw_encode_ext) + prIWEncExt->key_len))
		return -EINVAL;
	if (FALSE == GLUE_CHK_PR3(prNetDev, prEnc, pcExtra))
		return -EINVAL;
	prGlueInfo = *((P_GLUE_INFO_T *) netdev_priv(prNetDev));
