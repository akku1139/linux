...
VOID swCtrlCmdCategory0(P_ADAPTER_T prAdapter, UINT_8 ucCate, UINT_8 ucAction, UINT_8 ucOpt0, UINT_8 ucOpt1)
{
	...
	if (ucRead == SWCR_WRITE) {
		...
		case SWCTRL_DEBUG:
#if DBG
			if (ucOpt0 >= DBG_MODULE_NUM) {
				DBGLOG(SW4, WARN, "ucOpt0 %d invalid\n", ucOpt0);
				break;
			}
...
/* rStatus %x\n", g_u4mDNSRXFilter, ucOpt0, ucOpt1, fgUpdate, u4rxfilter, rStatus)); */
...
#if DBG
			if (ucOpt0 >= DBG_MODULE_NUM) {
				DBGLOG(SW4, WARN, "ucOpt0 %d invalid", ucOpt0);
				break
			}
		...
		case SWCTRL_QM_INFO:
			{
				...
				case 0:
					if (ucOpt1 >= TC_NUM) {
						DBGLOG(SW4, WARN, "ucOpt1 %d invalid", ucOpt1);
						break;
					}
		...
		case SWCTRL_DUMP_BSS:
			{
				if (ucOpt0 >= NETWORK_TYPE_INDEX_NUM) {
					DBGLOG(SW4, WARN, "ucOpt0 %d invalid", ucOpt0);
					break;
				}
