	...
	*pu4SetInfoLen = sizeof(PARAM_CUSTOM_SW_CTRL_STRUCT_T);
	...
	case 0x1001:
		if (u2SubId == 0x0)
			...
			if ((u4Data >> 16) >= NETWORK_TYPE_INDEX_NUM) {
				DBGLOG(INIT, ERROR, "BSS index (%d) out of bound\n", u4Data >> 16);
				return WLAN_STATUS_INVALID_DATA;
			}
    ...
    )
{
    ...
    *pu4SetInfoLen = 0;
	...
	if (prAdapter->fgEnArpFilter)
	{
		...
		kalMemZero(prCmdIpv6NetworkAddressList, u4CmdSize);
			  ...
			  OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	kalMemZero(&rCmdGetBuildDateCode, sizeof(CMD_GET_BUILD_DATE_CODE));
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (!prAdapter || !pvSetBuffer)
		return WLAN_STATUS_INVALID_DATA;
	kalMemZero(&rSuspendCmd, sizeof(WIFI_SYSTEM_SUSPEND_CMD_T));
