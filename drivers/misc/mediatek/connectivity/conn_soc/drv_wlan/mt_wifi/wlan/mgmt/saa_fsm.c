	...
	case SAA_STATE_WAIT_AUTH2:
		...
		if (authCheckRxAuthFrameStatus(prAdapter,
			...
			if (u2StatusCode == STATUS_CODE_SUCCESSFUL) {
				...
				DBGLOG(SAA, INFO, "Auth Req was rejected, Status Code = %d\n", u2StatusCode);
				DBGLOG(SAA, TRACE, "Auth Req was rejected by [ %pM ]\n", (prStaRec->aucMacAddr));
				eNextState = AA_STATE_IDLE;
			}
...
WLAN_STATUS saaFsmRunEventRxDeauth(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	DBGLOG(SAA, INFO, "Rx Deauth frame.\n");
	DBGLOG(SAA, TRACE, "Rx Deauth frame from BSSID=[ %pM ].\n", prDeauthFrame->aucBSSID);
