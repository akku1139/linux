...
static const UINT_16 g_u2CountryGroup1[] = {
	COUNTRY_CODE_AS, COUNTRY_CODE_AI, COUNTRY_CODE_BM, COUNTRY_CODE_VI,
...
static const UINT_16 g_u2CountryGroup2[] = {
	...
	COUNTRY_CODE_UY, COUNTRY_CODE_VN, COUNTRY_CODE_CA, COUNTRY_CODE_MX
};
...
DOMAIN_INFO_ENTRY arSupportedRegDomains[] = {
	...
	,
	{
	 (PUINT_16) g_u2CountryGroup2, sizeof(g_u2CountryGroup2) / 2,
	 {
	  ...
	  ,			/* CH_SET_UNII_UPPER_100_116 */
	  {121, BAND_5G, CHNL_SPAN_20, 132, 4, FALSE}
	  ,			/* CH_SET_UNII_UPPER_132_144 */
#endif
	  {125, BAND_5G, CHNL_SPAN_20, 149, 5, FALSE}
	  }                     /* CH_SET_UNII_UPPER_149_165 */
	 }
...
static UINT_16 g_u2CountryGroup2_Passive[] = {
	...
	COUNTRY_CODE_GB, COUNTRY_CODE_AT, COUNTRY_CODE_US, COUNTRY_CODE_MX,
	COUNTRY_CODE_JP, COUNTRY_CODE_IN, COUNTRY_CODE_CA
};
#endif
/*
...
DOMAIN_INFO_ENTRY arSupportedRegDomains_Passive[] = {
	...
	},
	{
		...
		ARRAY_SIZE(g_u2CountryGroup2_Passive),
		{
			...
			{115, BAND_5G, CHNL_SPAN_20, 36, 0, 0},
			{118, BAND_5G, CHNL_SPAN_20, 52, 4, 0},    /* 52~64 */
			{121, BAND_5G, CHNL_SPAN_20, 100, 12, 0},  /* 100~144 */
...
static struct reg_mapping support_region[] = {
	...
	{ COUNTRY_CODE_CA, REGION_CODE_FCC, SUPPORT_ALL},
	...
	{ COUNTRY_CODE_MX, REGION_CODE_FCC, SUPPORT_RADAR},
};
#endif
...
#define IN_LOCALE       "en-IN"
	...
	if (strncmp(buf, IN_LOCALE, strlen(IN_LOCALE)) == 0)
		ret = TRUE;
...
#if CFG_CUSTOM_REG
	if ((kalStrnCmp(CONFIG_ARCH_MTK_PROJECT, "radar", 5)) &&
			(kalStrnCmp(CONFIG_ARCH_MTK_PROJECT, "biscuit", 7))) {
		/* Non RADAR & BISCUIT projects*/
		ASSERT(prDomainInfo);

		for (i = 0; i < MAX_SUBBAND_NUM; i++) {
			/* Disable DFS channels. */
			if ((prDomainInfo->rSubBand[i].ucRegClass >= 118) &&
				(prDomainInfo->rSubBand[i].ucRegClass <= 121))
				prDomainInfo->rSubBand[i].ucNumChannels = 0;

		}
		/* Tablet project EU group no support UNI-3 band. */
		if (prDomainInfo->pu2CountryGroup == g_u2CountryGroup5) {
			ASSERT(prDomainInfo->rSubBand[4].ucRegClass == 125);
			prDomainInfo->rSubBand[4].ucNumChannels = 0;
		}
	}
#endif
