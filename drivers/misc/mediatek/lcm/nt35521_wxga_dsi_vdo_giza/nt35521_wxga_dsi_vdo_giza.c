...
static void init_giza_evt_txd_lcm(void)
{
...
static void init_giza_hvt_hsd_lcm(void)
{
	unsigned int data_array[64];
...
static void init_giza_hvt_inx_lcm(void)
{
	unsigned int data_array[64];
...
static char *lcm_get_vendor_type(void)
{
	...
	case HSD: return "HSD\0";
	case TXD: return "TXD\0";
	default: return "Unknown\0";
	}
}
...
static void lcm_init(void)
{
	...
	if(vendor_id == TXD)
		init_giza_evt_txd_lcm(); /* TXD panel */
	else if(vendor_id == HSD)
		init_giza_hvt_hsd_lcm(); /* HSD panel */
	else
		init_giza_hvt_inx_lcm(); /* INX panel */
...
LCM_DRIVER nt35521_wxga_dsi_vdo_giza_lcm_drv = {
	.name			= "nt35521_wxga_dsi_vdo_giza",
