...
s32 gtp_get_fw_info(struct i2c_client *, struct goodix_fw_info *fw_info);
...
static ssize_t gt91xx_noise_read_proc(struct file *file,
	char __user *buffer, size_t size, loff_t *ppos);

static struct proc_dir_entry *gt91xx_noise_proc = NULL;
static const struct file_operations noise_proc_ops = {
	.owner = THIS_MODULE,
	.read = gt91xx_noise_read_proc,
	.write = NULL,
};
...
static ssize_t gt91xx_fw_version_read_proc(struct file *file, char __user *buffer, size_t size, loff_t *ppos)
{
	...
	ret = gtp_get_fw_info(i2c_connect_client, fw_info);

	if (ret < 0)
		ptr += snprintf(ptr, PAGE_SIZE, "%s", "Read version failed\n");
	else
		ptr += snprintf(ptr, PAGE_SIZE, "0x%02x\n", fw_info->version);
...
static ssize_t gt91xx_noise_read_proc(struct file *file, char __user *buffer, size_t size, loff_t *ppos)
{
	int i, ret;
	char *ptr = NULL;
	char *page = NULL;
	size_t data_len = 0;
	char noise_data[GTP_MAX_SCAN_FREQS + 2] = {(u8)(GTP_REG_NOISE >> 8),
						(u8)(GTP_REG_NOISE & 0xFF)};
	char work_freq[4] = {(u8)(GTP_REG_WORK_FREQ >> 8), (u8)(GTP_REG_WORK_FREQ & 0xFF)};
	char start_freq[3] = {(u8)(GTP_REG_START_FREQ >> 8), (u8)(GTP_REG_START_FREQ & 0xFF)};
	char end_freq[3] = {(u8)(GTP_REG_END_FREQ >> 8), (u8)(GTP_REG_END_FREQ & 0xFF)};
	int total_supported_freqs = 0;
	size_t read_freqs = 0;
	struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client);
	...
	page = kmalloc(PAGE_SIZE, GFP_KERNEL);
	if (!page) {
		dev_err(&ts->client->dev, "Failed alloc memory for reading noise data\n");
		return -ENOMEM;
	}
	ptr = page;

	// Read current work frequency data (two bytes)
	ptr += snprintf(ptr, PAGE_SIZE, "Current work frequency: ");
	ret = gtp_i2c_read(i2c_connect_client, work_freq, 4);
	if (ret < 0)
		goto out;
	else
		ptr += snprintf(ptr, PAGE_SIZE, "0x%02X 0x%02X\n", work_freq[2], work_freq[3]);

	// Read start and end frequency to calulate total supported frequency number
	ptr += snprintf(ptr, PAGE_SIZE, "Total supported freqs: ");
	ret = gtp_i2c_read(i2c_connect_client, start_freq, 3);
	if (ret < 0)
		goto out;
	else
		ptr += snprintf(ptr, PAGE_SIZE, "(0x%2X:", start_freq[2]);
	ret = gtp_i2c_read(i2c_connect_client, end_freq, 3);
	if (ret < 0)
		goto out;
	else
		ptr += snprintf(ptr, PAGE_SIZE, "0x%2X)", end_freq[2]);
	// Calculate total supported hopping frequencies
	total_supported_freqs = end_freq[2] - start_freq[2] + 1;
	ptr += snprintf(ptr, PAGE_SIZE, " %d freqs\n", total_supported_freqs);

	ptr += snprintf(ptr, PAGE_SIZE, "Noise value:\n");
	// Iterate to read noise for all supported frequencies
	while (total_supported_freqs > 0) {
		// We can read up to GTP_MAX_SCAN_FREQS freqs for one time
		read_freqs = total_supported_freqs < GTP_MAX_SCAN_FREQS ?
				total_supported_freqs : GTP_MAX_SCAN_FREQS;
		ret = gtp_i2c_read(i2c_connect_client, noise_data, read_freqs + 2);
		if (ret < 0) {
			goto out;
		} else {
			for (i = 0; i < read_freqs; i++) {
				ptr += snprintf(ptr, PAGE_SIZE, "0x%02X ", noise_data[i + 2]);
				if (i % 10 == 9)
					ptr += snprintf(ptr, PAGE_SIZE, "\n");
			}
		}
		total_supported_freqs -= read_freqs;
	}
	ptr += snprintf(ptr, PAGE_SIZE, "\n");

out:
	data_len = ptr - page;
	ret = copy_to_user(buffer, (char *)page, data_len);
	if (ret) {
		kfree(page);
		return ret;
	} else{
...
= BIT_MASK(BTN_TOUCH);
	...
	ts->input_dev->id.vendor = ts->fw_info.sensor_id;
	ts->input_dev->id.product = 0xBEEF;
	ts->input_dev->id.version = ts->fw_info.version;
...
/*  HZ: clock ticks in 1 second generated by system */
	...
	if (ret < 0)
		GTP_ERROR("Read fw version failed.");
	...
	gt91xx_noise_proc
		= proc_create(GT91XX_NOISE_PROC_FILE,
				0444, NULL, &noise_proc_ops);

	if (gt91xx_noise_proc == NULL)
		GTP_ERROR("create_proc_entry %s failed\n",
				GT91XX_NOISE_PROC_FILE);
	else
		GTP_INFO("create proc entry %s success",
				GT91XX_NOISE_PROC_FILE);
